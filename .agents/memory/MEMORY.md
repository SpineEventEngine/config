...
- [pom-report-per-project-collectors](project/pom-report-per-project-collectors.md) — `generatePom` collects resolved versions via per-project `collectResolvedVersions` tasks; root-task resolution and captured `rootComponent` Providers both fail Gradle's lock check.
