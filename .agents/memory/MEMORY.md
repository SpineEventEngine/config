...
- [gradle-10-third-party-deprecations](project/gradle-10-third-party-deprecations.md) — Gradle 9.6 deprecation nags come from Detekt, Kover, and Gradle Doctor, not our build logic; Kover is fixed in 0.9.9 (bump via `dependency-update`), Detekt and Gradle Doctor still pending — don't chase them in `buildSrc`.
